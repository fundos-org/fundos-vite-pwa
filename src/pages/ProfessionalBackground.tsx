    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (data.success || response.ok) {
                toast.success(data.message || 'Professional background submitted successfully');
                navigate(eRoutes.USER_DETAILS_AUTH);
            } else {
                toast.error('Failed to submit professional background. Please try again.');
            }
        } catch (error) {
    ...
    const renderDropdown = (name: keyof typeof options, label: string) => {
        ...
        `}
          ...
          "
              ...
              ${formData[name] === option.value ? 'bg-gray-600 text-white' : 'bg-transparent text-white hover:bg-gray-600'}
            `}
                                >
                                    {option.label}
                                </button>
